...
const ResultsView: React.FC<ResultsViewProps> = ({ score, totalQuestions, hasWrongAnswers, onRestart, onReview, hasNextBatch, onNextBatch }) => {
  ...
  const getFeedback = () => {
    if (percentage === 100) return { message: "Perfect Score!", icon: <TrophyIcon className="h-24 w-24 text-yellow-400"/>, color: "text-yellow-400" };
    if (percentage >= 80) return { message: "Excellent Work!", icon: <TrophyIcon className="h-24 w-24 text-green-400"/>, color: "text-green-400" };
    if (percentage >= 50) return { message: "Good Effort!", icon: <BrainIcon className="h-24 w-24 text-cyan-400"/>, color: "text-cyan-400" };
    return { message: "Keep Practicing!", icon: <BrainIcon className="h-24 w-24 text-slate-400"/>, color: "text-slate-400" };
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center p-8 bg-slate-800 rounded-xl shadow-2xl max-w-2xl mx-auto">
      ...
      <p className="text-slate-300 text-xl mb-4">You answered</p>
